...
def render_sheet_preview(sheet: dict):
    ...
    # Product image (optional) — data URI bypasses PIL/WebP compatibility issues
    img_b64 = sheet.get("product_image_b64")
    if img_b64:
        mime = sheet.get("product_image_mime", "image/jpeg")
        st.markdown(
            f'<img src="data:{mime};base64,{img_b64}" style="max-width:100%;" />',
